...
def update_battery():
    ...
    if cmd == "charge":
        ...
        b["power_kw"] = 0
        ambient = 25.0
        cooling_rate = 0.08  # fraction of gap-to-ambient lost per tick
        b["temperature"] -= (b["temperature"] - ambient) * cooling_rate
        b["temperature"] += random.uniform(-0.05, 0.05)  # small sensor noise only
